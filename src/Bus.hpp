...
class Bus {
 public:
  virtual uint32_t getBlockedCycles(std::array<std::shared_ptr<Cache>, 4>&& caches, uint8_t pid, CacheOp, uint32_t) = 0;
...
class MESIBus : public Bus {
 public:
  ...
  uint32_t getBlockedCycles(std::array<std::shared_ptr<Cache>, 4>&& caches, uint8_t pid, CacheOp, uint32_t) override;
